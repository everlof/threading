...
final class ExtensionRendererTests: XCTestCase {
  ...
  func testNativeValueControlsAndSceneRaiseCorrelatedValues() throws {
    var events: [(String, ExtensionJSONValue?)] = []
    let panel = try XCTUnwrap(ExtensionExperimentFixture.registration.panels.first)
    let host = try ExtensionNodeRenderer.render(panel.root) { actionID, value in
      events.append((actionID, value))
    }
    ...
    func testPickerWithNilSelectionRemainsUnselectedAndNamed() throws {
        let host = try ExtensionNodeRenderer.render(
            .picker(
                ...
                selection: nil,
                options: [
                    .init(value: "current", title: "Current"),
                    .init(value: "previous", title: "Previous")
                ],
        ...
        let picker = try XCTUnwrap(
            descendants(in: host).compactMap { $0 as? ThemedPopUp }.first
        )

        XCTAssertEqual(picker.indexOfSelectedItem, -1)
        XCTAssertNil(picker.accessibilityValue())
        XCTAssertEqual(picker.accessibilityTitle(), "Release")
    }

    func testComponentCustomizationRoutesAControlValue() throws {
        let target = ExtensionComponentTarget(
            component: "test.value-control",
            contractVersion: 1
        )
        let resolution = ComponentCustomizationResolution(
            properties: [:],
            slots: [:],
            ...
            replacementExtensionIdentifier: "com.example.navigator",
            replacementCandidates: ["com.example.navigator"],
            hooks: []
        )
        let container = ComponentContentContainer(defaultContent: NSView())
        var actions: [ComponentCustomizationAction] = []
        let customization = ComponentCustomizationHost(
            target: target,
            contentContainer: container,
            lookup: { _ in resolution },
            onAction: { actions.append($0) }
        )
        customization.refresh()

        let field = try XCTUnwrap(
            descendants(in: container).compactMap { $0 as? ThemedSearchField }.first
        )
        field.stringValue = "active"
    ...
    func testWorkspaceNavigatorRendersVirtualCollectionsAndMatchesCanonicalDestinations()
        throws
    {
        let projectID = UUID()
                ...
                items: [
                    .init(
                        ...
                        activation: .destination(
                            .project(id: projectID.uuidString.uppercased())
                        )
                    )
                ]
            ))
        ...
        XCTAssertTrue(descendants(in: host.view).contains {
            $0.accessibilityIdentifier()
                == "workspace.navigator.item.project-list.project-row"
        })

        host.synchronizeSelection(with: .project(
    ...
    func testWorkspaceNavigatorSelectionNoneStillActivatesWithoutRetainingSelection()
        throws
    {
        var destinations: [ExtensionWorkspaceNavigatorDestination] = []
        let destination = ExtensionWorkspaceNavigatorDestination.project(
            id: UUID().uuidString
        )
        let navigator = ExtensionWorkspaceNavigator(
            id: "actions",
            ...
            root: .collection(.init(
                ...
                items: [
                    .init(
                        id: "open-project",
                        content: .text("Open project", role: .body),
        ...
        XCTAssertEqual(destinations, [destination])
        XCTAssertEqual(outline.selectedRow, -1)
    }

    func testWorkspaceNavigatorGridActionCarriesItemIDAndAtomicallyReplacesDocument()
        throws
    {
        let navigator = ExtensionWorkspaceNavigator(
            id: "activity",
            ...
            root: .collection(.init(
                ...
                items: [
                    .init(
                        ...
                        accessibilityLabel: "Open card one",
                        activation: .action(id: "open-card")
                    )
                ]
            ))
        )
        ...
        router.result = .success(.init(
            requestID: "ignored-by-ui-router",
        ...
        let host = WorkspaceNavigatorHostViewController(
            ...
            routing: router,
            contextProvider: {
                .init(projectID: "project-1", sessionID: "session-1")
            },
            destinationHandler: { _ in nil },
            onUnavailable: { XCTFail("valid navigator became unavailable") }
        )
        _ = host.view
        host.view.frame = NSRect(x: 0, y: 0, width: 320, height: 240)
        host.view.layoutSubtreeIfNeeded()
        ...
        let cell = try XCTUnwrap(
            descendants(in: host.view).compactMap { $0 as? NavigatorGridItemView }.first
        )
        XCTAssertEqual(cell.accessibilityLabel(), "Open card one")
        XCTAssertTrue(cell.accessibilityPerformPress())

        XCTAssertEqual(router.invocations.count, 1)
        XCTAssertEqual(router.invocations.first?.actionID, "open-card")
        XCTAssertEqual(router.invocations.first?.value, .string("card-1"))
        XCTAssertEqual(
    ...
    func testWorkspaceNavigatorRenderFailureFallsBackToNativeForThatProcessGeneration() {
        let invalid = ExtensionWorkspaceNavigator(
            id: "broken",
            title: "",
            root: .content(.text("Broken", role: .body))
        )
        let router = TestWorkspaceNavigatorRouter(navigator: invalid)
        ...
        let failedBack = expectation(description: "deferred native failback")
    ...
    func testInvalidWorkspaceNavigatorRuntimeResponseFailsClosed() throws {
        ...
        let router = TestWorkspaceNavigatorRouter(navigator: navigator)
        router.result = .failure(ExtensionProcessError.invalidMessage("invalid snapshot"))
        ...
        let host = WorkspaceNavigatorHostViewController(
            ...
            contextProvider: { .init() },
            destinationHandler: { _ in nil },
            onUnavailable: { becameUnavailable = true }
        )
        _ = host.view
    ...
    func testStaleNavigatorFailureCannotEvictAReplacementProcessGeneration() throws {
        let navigator = ExtensionWorkspaceNavigator(
            id: "activity",
            title: "Activity",
            root: .content(.button(
        ...
        container.activate(selection)
        let oldHost = try XCTUnwrap(
            container.children.compactMap {
                $0 as? WorkspaceNavigatorHostViewController
            }.first
        )
        let button = try XCTUnwrap(
            descendants(in: oldHost.view).compactMap { $0 as? ThemedButton }.first
        )
        button.performClick()
        ...
        router.completeDeferred(with: .failure(ExtensionProcessError.notRunning))

        XCTAssertEqual(container.effectiveSelection, selection)
        XCTAssertEqual(
            container.children.compactMap {
                $0 as? WorkspaceNavigatorHostViewController
    ...
    func testWorkspaceNavigatorSelectionPersistsAndNativeClearsTheOverride() throws {
        ...
        AppSettings(defaults: defaults).workspaceNavigatorSelection = selection
        XCTAssertEqual(
            AppSettings(defaults: defaults).workspaceNavigatorSelection,
            selection
        )

        AppSettings(defaults: defaults).workspaceNavigatorSelection = .native
        XCTAssertEqual(
            AppSettings(defaults: defaults).workspaceNavigatorSelection,
            .native
        )
  ...
  func testArtifactSceneWritesAReviewableScreenshot() throws {
    ...
    let panel = try XCTUnwrap(ExtensionExperimentFixture.registration.panels.first)
    let rendered = try ExtensionNodeRenderer.render(panel.root) { _, _ in }
    let width: CGFloat = 620
    rendered.widthAnchor.constraint(equalToConstant: width).isActive = true
    rendered.layoutSubtreeIfNeeded()
    let height = max(rendered.fittingSize.height, 520)
    rendered.frame = NSRect(
      origin: .zero,
      size: NSSize(width: width, height: height)
    )
    rendered.layoutSubtreeIfNeeded()
    rendered.wantsLayer = true
    rendered.layer?.backgroundColor = Design.Surface.ground.cgColor

    let representation = try XCTUnwrap(
      rendered.bitmapImageRepForCachingDisplay(in: rendered.bounds)
    )
    rendered.cacheDisplay(in: rendered.bounds, to: representation)
    ...
    try png.write(
      to: directory.appendingPathComponent("extension-artifact-scene.png")
    )
  }
  ...
  func testBrokeredCacheStorageRoundTripsAndRefusesTraversalAndOversizedEntries() throws {
    ...
    let observed = try asAnExtensionProcess {
      () -> (miss: Bool, names: [String], value: Data?, afterRemoval: [String]) in
  ...
  func testHostClientStoresSecretsInAnExtensionScopedBroker() async throws {
    ...
    let readOnlyAuthorization = try XCTUnwrap(
      try service.authorize(
  ...
  func testHostSnapshotsRequireCapabilitiesAndRedactRepositoryMetadataSeparately() throws {
    ...
    let projectsOnly = try XCTUnwrap(
      try service.authorize(
    ...
    let withRepository = try XCTUnwrap(
      try service.authorize(
  ...
  func testSessionRuntimeBrokerIsSeparatelyGatedAndBoundToAKnownSession() throws {
    ...
    let sessionsOnly = try XCTUnwrap(
      try service.authorize(
    ...
    let runtimeReader = try XCTUnwrap(
      try service.authorize(
  ...
  func testConversationRowHooksRetainNativeStateAndPermissionAuthority() throws {
    ...
    for change in controller.timeline.apply(
      .toolResults([
  ...
  func testProductionExtensionPanelRoutesAControlValue() throws {
    ...
    let router = TestExtensionPanelRouter(
      item: .init(
        extensionIdentifier: "codes.threading.artifact-preview",
        extensionName: "ArtifactKit Preview",
        processGeneration: "generation-one",
        panel: panel
      )
    )
    router.result = .success(.init(requestID: "response", panel: panel))
    let context = ExtensionCommandContext(
      projectID: "project-1",
      sessionID: "session-1"
    )
  ...
  func testProductionExtensionPanelHostsCompanionPixelsWithSemanticFallback() throws {
    ...
    XCTAssertTrue(
      surface.remoteSurfaceDidReceive(
        .init(
          metadata: .init(
  ...
  func testSemanticSceneTabPersistsTheRendererNeutralDocument() throws {
    ...
    XCTAssertEqual(decoded.tabs.first?.kind, .semanticScene)
    XCTAssertEqual(decoded.tabs.first?.semanticScene, scene)
    XCTAssertTrue(decoded.signature.contains("iOS 26.5 vs 26.4"))
    XCTAssertTrue(
      decoded.agentDescription.contains(
        "native semantic visualization \"iOS 26.5 vs 26.4\""
      )
    )
  }
...
private final class TestWorkspaceNavigatorRouter: ExtensionWorkspaceNavigatorRouting {
    ...
    func completeDeferred(
        with result: Result<ExtensionWorkspaceNavigatorActionResponse, Error>
    ) {
