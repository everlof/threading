...
final class TerminalDropTests: XCTestCase {
    ...
    func testAPasteboardIsJudgedWithoutWritingAnything() throws {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("threading.terminal.drop.test"))
        ...
        pasteboard.clearContents()
        let url = URL(fileURLWithPath: "/tmp/threading-drop-fixture.png")
    ...
    func testRawImageDataBecomesAFileTheAgentCanOpen() throws {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("threading.terminal.drop.image"))
        pasteboard.clearContents()
