...
final class ExtensionBundleLoaderTests: XCTestCase {
    ...
    func testInspectorSaysWhichSideIsOutOfDateForAnUnknownManifestFormat() throws {
        func inspect(_ manifest: String) -> ExtensionBundleError? {
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("SkalmanFormat-\(UUID().uuidString)", isDirectory: true)
        ...
        // A package from a future Skalman. It may carry fields today's decoder cannot read at
        ...
        XCTAssertTrue(
            future?.errorDescription?.contains("needs a newer version of Skalman") == true,
    ...
    func testPackagerRequiresAndPlacesEveryDeclaredCompanionApp() throws {
        ...
        let destination = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "SkalmanPackagedCompanion-\(UUID().uuidString).skalmanextension",
                isDirectory: true
            )
    ...
    func testPackagerRejectsAnAppForAnUndeclaredCompanion() throws {
        ...
        let destination = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "SkalmanUnexpectedCompanion-\(UUID().uuidString).skalmanextension",
                isDirectory: true
            )
        defer { try? FileManager.default.removeItem(at: destination) }
    ...
    func testCompanionSupervisorUsesOnlyItsOwnGenerationBoundControlChannel() throws {
        ...
        XCTAssertEqual(request.arguments, ["--skalman-companion-serve"])
        XCTAssertEqual(
    ...
    func testSimulatorRelayDogfoodProducesARealFrameAndAcceptsInput() throws {
        ...
        XCTAssertEqual(bundle.manifest.identifier, "se.mjukis.simulator-relay")
        let companion = try XCTUnwrap(bundle.companions.first {
        ...
        supervisor.startRemoteSurfaces { packet in
            ...
            if frame.sequence == 1 {
                let outputPath = ProcessInfo.processInfo.environment[
                    "SKALMAN_SIMULATOR_RELAY_FRAME_PATH"
                ] ?? "/tmp/skalman-simulator-relay-frame.png"
                do {
                    try Self.writePNG(
                        bgra: packet.payload,
                        width: frame.width,
    ...
    func testManagerStartsBackgroundCompanionsAndLeavesOnDemandWorkersDormant() async throws {
        ...
        let storeRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "SkalmanCompanionManager-\(UUID().uuidString)",
                isDirectory: true
            )
        defer {
    ...
    func testWebAssemblyLaunchPolicyPassesTheValidatedModuleAsDescriptorFour() throws {
        ...
        let runnerDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanFakeWasmRunner-\(UUID().uuidString)")
        try FileManager.default.createDirectory(
            at: runnerDirectory,
            withIntermediateDirectories: true
        )
    ...
    func testBuiltAppShipsACompleteVersionedExtensionSDKSnapshot() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(
            atPath: sdk.appendingPathComponent(
                "Sources/SkalmanExtensionKit/ExtensionManifest.swift"
            ).path
        ))
        for requiredPath in [
            "AGENT_AUTHORING.md",
    ...
    func testSandboxLaunchFailsClosedWhenTheWrapperIsUnavailable() throws {
        ...
        XCTAssertThrowsError(try ExtensionSandboxPolicy.launch(
            bundle: bundle,
            commandArguments: ["--skalman-serve"],
            environment: [:],
    ...
    func testRegistrationAndServeShareOneLaunchAbstraction() throws {
        ...
        XCTAssertEqual(probe.requests.first?.arguments, ["--skalman-register"])
        ...
        XCTAssertThrowsError(try ExtensionProcessSession.start(
            bundle: bundle,
            policy: serve,
            additionalEnvironment: ["SKALMAN_EXTENSION_HOST_TOKEN": "abc"]
        ))
        XCTAssertEqual(serve.requests.first?.arguments, ["--skalman-serve"])
        XCTAssertEqual(
            serve.requests.first?.additionalEnvironment["SKALMAN_EXTENSION_HOST_TOKEN"],
            "abc"
        )
    ...
    func testTheChildEnvironmentIsTheHostBasePlusHostAdditions() {
        let composed = ExtensionLaunchEnvironment.composed(
            with: ["SKALMAN_EXTENSION_HOST_URL": "http://127.0.0.1:1"]
        )
        XCTAssertEqual(composed["SKALMAN_EXTENSION_PROTOCOL"], "1")
        XCTAssertEqual(composed["PATH"], "/usr/bin:/bin:/usr/sbin:/sbin")
        XCTAssertEqual(composed["SKALMAN_EXTENSION_HOST_URL"], "http://127.0.0.1:1")
        XCTAssertNil(composed["HOME"])
        XCTAssertNil(composed["SKALMAN_MCP_PORT"])
    }
    ...
    func testTheSandboxDeniesEverythingAnExtensionDidNotDeclare() throws {
        let outside = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanProbeOutside-\(UUID().uuidString)")
        try Data("secret".utf8).write(to: outside)
        defer { try? FileManager.default.removeItem(at: outside) }

        let escape = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanProbeEscape-\(UUID().uuidString)")
    ...
    func testTheSandboxEndsAnExtensionThatTriesToRunAnotherProgram() throws {
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanProbeSpawned-\(UUID().uuidString)")
    ...
    func testTheSpawnerInstallsABrokerDescriptorPastTheStandardStreams() throws {
        ...
        let child = try ExtensionChildSpawner.spawn(
            ...
            arguments: [bundle.executableURL.path, "--skalman-serve"],
            environment: ["SKALMAN_PROBE": "present"],
        ...
        XCTAssertTrue(output.contains("argument=--skalman-serve"), output)
    ...
    func testTheSignedHelperRunsAnExtensionAndContainsIt() throws {
        ...
        let package = URL(fileURLWithPath: installRoot, isDirectory: true)
            .appendingPathComponent(
                "se.mjukis.skalmantest.\(UUID().uuidString).skalmanextension",
                isDirectory: true
            )
        defer { try? FileManager.default.removeItem(at: package) }

        let escape = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanHelperEscape-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: escape) }
        ...
        let manifest = ExtensionManifest(
            identifier: "se.mjukis.skalmantest.helper",
            name: "Helper Probe",
            version: "0.1.0",
        ...
        XCTAssertTrue(report.contains("entryMode=--skalman-serve"), transcript)
    ...
    func testTheCompiledProbeIsContainedByTheSignedHelper() throws {
        ...
        let helper = try Self.resigned(
            ...
            entitlements: repository
                .appendingPathComponent("Helper/skalman-extension-helper.entitlements")
        )
        defer { try? FileManager.default.removeItem(at: helper.deletingLastPathComponent()) }
        let probe = repository.appendingPathComponent(
            "SkalmanExtensionKit/.build/\(Self.hostTriple)/debug/DenialProbeExtension"
        )
        try XCTSkipUnless(
            FileManager.default.isExecutableFile(atPath: probe.path),
            "build it first: swift build --package-path SkalmanExtensionKit "
        ...
        let package = URL(fileURLWithPath: installRoot, isDirectory: true)
            .appendingPathComponent(
                "se.mjukis.skalmantest.probe.\(UUID().uuidString).skalmanextension",
                isDirectory: true
            )
        defer { try? FileManager.default.removeItem(at: package) }
        try FileManager.default.createDirectory(
        ...
        try JSONEncoder().encode(ExtensionManifest(
            identifier: "se.mjukis.skalmantest.probe",
            name: "Denial Probe",
            version: "1.0.0",
            executable: "bin/extension",
            capabilities: [.panels]
        ...
        try KeychainExtensionSecretStore.shared.setData(
            ...
            extensionIdentifier: "se.mjukis.skalmantest.probe",
            key: secretKey
        )
        defer {
    ...
    func testTheSandboxPreventsAnAdversarialExtensionFromRaisingAKeychainPrompt() throws {
        ...
        let package = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "se.mjukis.skalmantest.adversarial-keychain.\(UUID().uuidString)"
                    + ".skalmanextension",
                isDirectory: true
            )
        defer { try? FileManager.default.removeItem(at: package) }
        ...
        let child = try SandboxExecLaunchPolicy().spawn(
            ExtensionLaunchRequest(
                ...
                additionalEnvironment: [
                    "SKALMAN_EXTENSION_ADVERSARIAL_KEYCHAIN_PROBE": "1"
                ],
                standardOutput: .pipe(stdout),
                standardError: .pipe(stderr)
            )
    ...
    private static func resigned(
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanHelperResign-\(UUID().uuidString)", isDirectory: true)
    ...
    func testTheRunnerRefusesAnythingItWasNotAskedToLaunch() throws {
        let installRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanRunnerPackages-\(UUID().uuidString)")
        try FileManager.default.createDirectory(
            at: installRoot,
            withIntermediateDirectories: true
        )
        ...
        let package = installRoot
            .appendingPathComponent("com.example.runner.skalmanextension", isDirectory: true)
        ...
        func request(
            ...
            arguments: [String] = ["--skalman-serve"],
        ...
        XCTAssertNil(refusal(request(arguments: ["--skalman-register"])))
        ...
        XCTAssertEqual(
            refusal(request(arguments: ["--skalman-serve", "--and-this"])),
            .unknownEntryMode(["--skalman-serve", "--and-this"])
        )
        ...
        let elsewhere = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanElsewhere-\(UUID().uuidString).skalmanextension")
        try FileManager.default.createDirectory(
            at: elsewhere,
            withIntermediateDirectories: true
        )
    ...
    private final class RecordingCoreLaunchPolicy: ExtensionLaunchPolicy {
    ...
    private final class RecordingCompanionChild: ExtensionChildProcess {
    ...
    private final class RecordingLaunchPolicy: ExtensionLaunchPolicy {
    ...
    private func makeBundle(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanExtensionLoaderTests-\(UUID().uuidString)")
    ...
    private func makeWebAssemblyBundle(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanWasmLoaderTests-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin", isDirectory: true)
        try FileManager.default.createDirectory(
            at: bin,
            withIntermediateDirectories: true
