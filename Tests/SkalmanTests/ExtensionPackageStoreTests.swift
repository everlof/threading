...
final class ExtensionPackageStoreTests: XCTestCase {
    ...
    func testMCPCoreDependsOnlyOnTheExternalProviderBoundary() throws {
        let sourceRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("Sources/Skalman/Core/MCP", isDirectory: true)
        ...
        XCTAssertFalse(source.contains("import SkalmanExtensionKit"))
    ...
    func testThePackagerAssemblesAnInstallablePackageFromAManifestAndABinary() throws {
        ...
        let manifestURL = workspace.appendingPathComponent("skalman-extension.json")
    ...
    func testScaffolderCreatesAnAtomicSelfContainedExtensionProject() throws {
        let sdk = try XCTUnwrap(Bundle.main.resourceURL?
            .appendingPathComponent("ExtensionSDK/SkalmanExtensionKit", isDirectory: true))
        ...
        XCTAssertTrue(FileManager.default.fileExists(
            atPath: destination.appendingPathComponent(
                "Vendor/SkalmanExtensionKit/Sources/SkalmanExtensionKit/ExtensionManifest.swift"
            ).path
        ))
        XCTAssertTrue(FileManager.default.fileExists(
            atPath: destination.appendingPathComponent(
        ...
        XCTAssertTrue(packageScriptSource.contains("com.example.build-watch.skalmanextension"))
        XCTAssertFalse(FileManager.default.fileExists(
            atPath: destination.appendingPathComponent(
                "Vendor/SkalmanExtensionKit/.build"
            ).path
        ))
    ...
    func testInstallProposalNamesTrustRuntimeCapabilitiesAndDisabledState() {
        ...
        let proposal = ExtensionInstallProposal(bundle: SkalmanExtensionBundle(
            rootURL: root,
    ...
    func testInstallProposalSeparatesCompanionOSAuthorityFromHostCapabilities() {
        ...
        XCTAssertTrue(proposal.message.contains("does not inherit additional Skalman host data"), proposal.message)
    }

    func testDogfoodScaffoldBuildsPackagesInstallsAndRunsAsWebAssembly() throws {
        let environment = ProcessInfo.processInfo.environment
        guard let swiftPath = environment["SKALMAN_SWIFT_WASM"],
              let sdkID = environment["SKALMAN_SWIFT_WASM_SDK"] else {
            throw XCTSkip(
                "Set SKALMAN_SWIFT_WASM and SKALMAN_SWIFT_WASM_SDK to run the real "
        ...
        packageEnvironment["SKALMAN_SWIFT_EXEC"] = swiftPath
    ...
    func testThePackagerLeavesNothingBehindWhenAssemblyFails() throws {
        ...
        let destination = workspace.appendingPathComponent("bad.skalmanextension")
        XCTAssertThrowsError(try ExtensionPackager.assemble(
            manifestURL: manifestURL,
            executableURL: built,
            into: destination
    ...
    func testThePackagerRefusesToOverwriteOrToEscapeThePackage() throws {
        ...
        for escaping in ["../outside", "/etc/passwd", "bin/../../outside"] {
            let destination = workspace.appendingPathComponent("\(UUID().uuidString).skalmanextension")
            XCTAssertThrowsError(try ExtensionPackager.assemble(
        ...
        let occupied = workspace.appendingPathComponent("taken.skalmanextension")
        ...
        XCTAssertThrowsError(try ExtensionPackager.assemble(
            ...
            into: workspace.appendingPathComponent("plain.skalmanextension")
        )) { error in
    ...
    private func makePackage(
        ...
        if !settingsResponseIDs.isEmpty {
            ...
            serveBody = """
                [ -n "$SKALMAN_EXTENSION_SETTINGS_JSON" ] || exit 68
                printf '%s\\n' "$SKALMAN_EXTENSION_SETTINGS_JSON" > "$SKALMAN_EXTENSION_KEY_VALUE_DIRECTORY/launch-settings.json"
                while IFS= read -r line; do
                  printf '%s\\n' "$line" >> "$SKALMAN_EXTENSION_KEY_VALUE_DIRECTORY/settings-requests.jsonl"
        ...
        let storageGuard = requiresStorageEnvironment
            ? """
                [ -n "$SKALMAN_EXTENSION_KEY_VALUE_DIRECTORY" ] || exit 66
                [ -n "$SKALMAN_EXTENSION_CACHE_DIRECTORY" ] || exit 67
                """
            : ""
        let script = """
            #!/bin/sh
    ...
    private func temporaryDirectory(_ label: String) -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "SkalmanExtensionPackageTests-\(label)-\(UUID().uuidString)",
            isDirectory: true
        )
