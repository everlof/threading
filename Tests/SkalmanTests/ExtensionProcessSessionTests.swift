...
final class ExtensionProcessSessionTests: XCTestCase {
    ...
    func testPersistentProcessRoutesACorrelatedCommandResponse() throws {
        ...
        let storage = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanExtensionCommand-\(UUID().uuidString)")
        ...
        let directory = try makeBundle(
            ...
            scriptAfterRegistration: """
            IFS= read -r request || exit 65
            printf '%s\\n' "$request" > "$SKALMAN_EXTENSION_KEY_VALUE_DIRECTORY/received-command.json"
            printf '%s\\n' \(shellQuoted(json(response)))
            while IFS= read -r request; do :; done
            """
        )
    ...
    func testPersistentProcessRoutesAComponentActionWithoutAppKit() throws {
        ...
        let storage = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanExtensionAction-\(UUID().uuidString)")
        try FileManager.default.createDirectory(
            at: storage,
            withIntermediateDirectories: true
        )
        ...
        let directory = try makeBundle(
            ...
            scriptAfterRegistration: """
            IFS= read -r request || exit 65
            printf '%s\\n' "$request" > "$SKALMAN_EXTENSION_KEY_VALUE_DIRECTORY/received-component-action.json"
            printf '%s\\n' \(shellQuoted(json(response)))
            while IFS= read -r request; do :; done
            """
        )
    ...
    func testSandboxKeepsThePackageReadOnlyAndDeclaredStorageWritable() throws {
        let storage = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanExtensionSandbox-\(UUID().uuidString)")
        try FileManager.default.createDirectory(
            at: storage,
            withIntermediateDirectories: true
        )
        ...
        let directory = try makeBundle(
            ...
            printf 'allowed' > "$SKALMAN_EXTENSION_KEY_VALUE_DIRECTORY/storage-write" || exit 68
            """,
    ...
    private func makeBundle(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkalmanExtensionProcessTests-\(UUID().uuidString)")
