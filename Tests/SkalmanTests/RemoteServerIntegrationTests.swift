...
final class RemoteServerIntegrationTests: XCTestCase {
    ...
    private func get(_ path: String, bearer: String? = nil, headers: [String: String] = [:]) -> Probe? {
        ...
        request.setValue("test-device", forHTTPHeaderField: "X-Skalman-Device")
    ...
    func testServesTheClientPageWithHardeningHeaders() throws {
        ...
        XCTAssertTrue(String(decoding: probe.body, as: UTF8.self).contains("Skalman Remote"))
    }
    ...
    func testTooOldClientProtocolGets426() throws {
        let probe = try XCTUnwrap(get(
            "/api/me",
            bearer: "goodtoken",
            headers: ["X-Skalman-Protocol": "0", "X-Skalman-Protocol-Min": "0"]
        ))
    ...
    func testTooNewClientProtocolAsksHostToUpdate() throws {
        let probe = try XCTUnwrap(get(
            "/api/me",
            bearer: "goodtoken",
            headers: ["X-Skalman-Protocol": "99", "X-Skalman-Protocol-Min": "99"]
        ))
        XCTAssertEqual(probe.status, 426)
    ...
    /// has seen 32 browser visits becomes permanently unavailable until Skalman restarts.
    ...
    func testValidBearerBypassesExhaustedFailedAuthBudget() throws {
        for attempt in 0..<RemoteAccessDefaults.failedAuthLimitGlobal {
            let probe = try XCTUnwrap(get(
                ...
                headers: ["X-Skalman-Device": "attacker-\(attempt)"]
            ))
            XCTAssertEqual(probe.status, 401)
        }
        ...
        XCTAssertEqual(
            try XCTUnwrap(get(
                "/api/me",
                bearer: "goodtoken",
                headers: ["X-Skalman-Device": "paired-phone"]
            )).status,
            200
        )
        XCTAssertEqual(
            try XCTUnwrap(get(
                "/api/me",
                bearer: "still-wrong",
                headers: ["X-Skalman-Device": "fresh-attacker"]
            )).status,
            429
        )
    }
