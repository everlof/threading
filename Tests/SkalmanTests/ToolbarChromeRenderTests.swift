...
/// rounded rect and three circles — which is visible in a picture and in nothing else.
///
/// Both backdrops are rendered because these views ink themselves against the *terminal's*
/// background rather than the chrome's ground (see `BackdropOverlay`), so a story drawn on one
/// tone says nothing about the other.
@MainActor
final class ToolbarChromeRenderTests: XCTestCase {
    ...
    private enum Render {
        ...
        /// A near-black terminal and a paper-white one: the two ends the ink is measured between.
        static let backdrops: [(name: String, colour: NSColor)] = [
            ("dark", NSColor(hex: "#0B0B0F")!),
            ("light", NSColor(hex: "#FAFAF7")!)
        ]
    }
