...
final class FileActivityMapRenderTests: XCTestCase {
    ...
    private var skalmanFiles: [String] {
        get throws {
            let url = URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent()
                .deletingLastPathComponent()
                .appendingPathComponent("Fixtures/FileLists/skalman-files.txt")
            try XCTSkipUnless(
    ...
    func testRendersTheStorybook() throws {
        ...
        for (offset, path) in files.filter({ $0.hasPrefix("Sources/Skalman/UI/Design/") }).enumerated() {
        ...
        for (offset, path) in files.filter({ $0.hasPrefix("Sources/Skalman/Core/Agent/") }).enumerated() {
            burst.record(.read, path: path, at: seconds(25 + offset))
            if offset.isMultiple(of: 2) {
                burst.record(.edit, path: path, at: seconds(1 + offset))
            }
        }
        burst.record(.edit, path: "Sources/Skalman/Core/Agent/BrandNewFile.swift", at: seconds(3))
        ...
        let ladder = files.filter { $0.hasPrefix("Sources/Skalman/UI/Preferences/") }
        ...
        let hovered = files.firstIndex { $0.hasPrefix("Sources/Skalman/Core/Agent/") } ?? 0
        written += try write(
