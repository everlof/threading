...
///
/// This file belongs only to the `SkalmanNotificationE2E` scheme. The ordinary `Skalman`
...
final class NotificationDeliveryE2ETests: XCTestCase {
    ...
    func testAPNsAcceptsPermissionNotification() async throws {
        ...
        let result = await configuration.sender.send(
            .init(
                ...
                title: "Skalman permission E2E \(nonce)",
    ...
    func testClaudeNotifyUserToolReachesAPNs() async throws {
        guard ProcessInfo.processInfo.environment["SKALMAN_E2E_RUN_CLAUDE"] == "1" else {
            throw XCTSkip(
        ...
        guard MCPToolCatalog.enabledToolNames.contains(MCPTools.notifyUser) else {
            throw XCTSkip(
                "The Notifications tool group is disabled. Enable it in Skalman’s Tools settings."
            )
        }

        let configuration = try NotificationE2EConfiguration.load()
        let nonce = NotificationE2EConfiguration.nonce()
        let expectedTitle = "Skalman Claude E2E \(nonce)"
        ...
        var session = AgentSession(
            ...
            model: ProcessInfo.processInfo.environment["SKALMAN_E2E_CLAUDE_MODEL"],
            usesNativeUI: true
        )
        ...
        guard stream.send(
            ...
            Call mcp__skalman__notify_user exactly once with title "\(expectedTitle)", \
...
private struct NotificationE2EConfiguration {
    ...
    static func load(
        ...
        guard let sender = RemoteAPNSPushSender.fromEnvironment(variables) else {
            throw XCTSkip(
                "Set SKALMAN_APNS_KEY_ID, SKALMAN_APNS_TEAM_ID and "
                    + "SKALMAN_APNS_PRIVATE_KEY_PATH to a valid APNs .p8 key."
            )
        }
        let token = variables["SKALMAN_E2E_APNS_DEVICE_TOKEN"]?
        ...
        guard !token.isEmpty, token.unicodeScalars.allSatisfy({
            ...
            throw XCTSkip(
                "Set SKALMAN_E2E_APNS_DEVICE_TOKEN to the hex token copied from the debug iOS app."
            )
        }
        let environmentName = variables["SKALMAN_E2E_APNS_ENVIRONMENT"] ?? "sandbox"
        guard let environment = RemoteAPNSPushSender.Environment(rawValue: environmentName) else {
            XCTFail("SKALMAN_E2E_APNS_ENVIRONMENT must be sandbox or production.")
