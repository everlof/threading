...
| `AccountUsageFleetView` / `UsageDashboardView` | The one-page Usage composition reads current capacity, Limit History, then Consumption. The fleet retains a constant-size summary over a bounded, virtualized account-card table shared by Settings and the Option-click toolbar popover; it names ready/constrained/unknown accounts and the next reset rather than averaging unlike windows. The dashboard follows with its limit chooser, reset/projection summary and independent-series chart, then a large total and top-three tool split beside the shared stacked-band chart, a five-metric strip, virtual breakdown and explicit coverage. Both accept prepared values; account discovery, filesystem scans, CLI exports, provider calls and journal I/O remain outside the design boundary. |
