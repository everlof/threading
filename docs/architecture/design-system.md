...
| `WindowTitleBandView` | The title band a chrome-takeover theme draws across the window's top (`WindowChromeStyle`, see [`window-chrome.md`](window-chrome.md)): the theme's gradient active and inactive, the window's title by observation, a leading slot for the rehomed toolbar controls, and the titlebar's own gestures — a press drags the window, a double-click performs the user's System Settings choice. Not a control (its `interactiveComponent` exception records why); its buttons are. |
| `WindowChromeButton` | A takeover window's close/minimize/zoom, one component for all three roles and both glyph styles (`squares` plates in the theme's control vocabulary, `plain` bare glyphs in the band's ink) — the tab strip's "every" lesson applied before there were two. Calls the *semantic* window operations, because the `perform*` forms animate a standard button a frameless window does not have and refuse outright. Full `ThemedControl` contract: keyboard, focus ring, AX press. |
