...
`ThemedControl` supplies accessibility hit testing for its drawn, cell-free controls. AppKit's
default `NSControl` implementation returns the cell; without one, a button could appear in the
accessibility tree with the correct title, frame and enabled state while returning no hit. The
native question journey exposed this when XCTest kept trying to scroll a visible Next button
...
a 620pt card became 620pt wide, putting the line counts outside the card. Set the column from the
requested frame before committing that frame: the first bounds read can flush a pending AppKit
tile and restore the old column width. Containment cannot infer this policy — the sidebar outline
is embedded too and deliberately retains AppKit's inset band. The conversation fixture and resize
regression cover the exact-width case.
...
| `SubagentSummaryView` | The compact child-agent navigator shared by the overview and transcript pane. In the `navigation` style every child is a `SubagentNavigatorRowView` (below); the inline style keeps its disclosure chevrons, because there the chevron really does open detail under the row. A row is a standing work receipt rather than a provider role alone: the name, then role/configuration/progress/usage on one quiet line, the delegated task when the name is not already made of it, and the latest distinct activity. Feature code supplies `SubagentSummaryItem.TranscriptAvailability` as one of three states: unavailable, openable from memory/while still running, or on disk with the file URL. A file-backed row is therefore structurally openable and revealable; pressability and the Finder action cannot disagree through independent Boolean/optional inputs. Optional usage arrives as already formatted semantic text, keeping provider accounting out of Design while adding no new rows beyond the navigator's existing 40-row page bound. The card's header holds its words in by the rows' ink inset so title, count and every name share one column. |
| `SubagentNavigatorRowView` | One child in the navigator as a **selectable row**, not a disclosure. The rows were buttons wearing a `›`, which promises more of *this* underneath; pressing one only swapped the transcript further down the pane, and the open row answered a second press with nothing. Three finished children were reported as "all agents just output to the same space" because no row said whose transcript was on screen. The row of the child on screen paints the theme's selection (`SelectionSurface.quiet`, the strength a themed list row paints) and inks its labels against that measured ground; a row that opens lifts under pointer, press and keyboard focus the way a tab does; a row that leads nowhere is disabled, keeps its place and facts, paints no plate and says why. The name is a label, not a button title: a theme's uppercase convention is for controls, and Codex's three `default` children drew as DEFAULT ×3. One accessible element per row — role, selected value, help — with the Finder mark its own control. |
| `SubagentTranscriptHeadingView` | The seam between the navigator and the transcript rows: the word *Transcript*, the child's name, its state and a rule, aligned by ink with the card above. In a pane with a dozen children the rows start a screen below the row that selected them, and without a name at the seam they read as a pool every child pours into. |
...
| `SemanticSceneView` | A bounded semantic visualization drawn from normalized marks. It is intentionally not a named chart or extension-specific tree: rectangles, rounded rectangles, and ellipses cover treemaps, heatmaps, timelines, scatter plots, and bubbles. Measured values belong in `ChartCardView` instead: this component is handed geometry and trusts it, which is the right contract for a caller that already has coordinates and the wrong one for a caller that has numbers. One `ThemedControl` canvas owns pointer, keyboard, hover, focus, enabled, and selected states; marks remain values instead of becoming as many as 500 AppKit subviews. Each mark is still a native virtual accessibility element, materialized only when AppKit asks for the group’s children. Callers supply semantic colour roles; the design system owns every pixel. Flat scenes keep translucent marks because overlap carries meaning there. Hierarchical circles instead use opaque, panel-derived Oklab fills: parents keep the theme's lightness variation while shedding chart-series saturation, leaves carry most of the colour, the focus gets a strong neutral perimeter, and internal construction lines stay ink-dark. Immediate-child labels are a final overlay pass, because they name the whole packed region rather than the last empty patch its descendants left behind. Alpha-compositing a child over its parent invented a third colour and made containment look like accidental overlap. A mark answers the pointer with the shape it drew, not with its bounding box: a circle fills only π/4 of its rectangle, and in a packing the corners it leaves are exactly where a sibling or the parent is what a person can see, so rectangle hit testing handed the highlight, the pointing hand, and the click to a mark the pointer was demonstrably not over. Hover and press repaint the affected marks alone, and a hierarchy fill is derived once per repaint per (role, depth, enabled, emphasis) — the derivation costs three surface composites and two Oklab round trips inside an appearance push, which a 500-mark scene was paying five hundred times every time the pointer crossed a boundary. |
| `NativePluginPresentationBoundaryView` | The non-drawing containment boundary around a native plugin's returned AppKit or SwiftUI tree. Plugins cannot link the private design system, so the host hands them public `PluginTheme` values and excludes only their exact presentation subtree from app-owned control auditing; host siblings remain audited. |
...
| `ThemedTimeSeriesChartView` / `ThemedStackedBandChartView` | Two reusable retained time-series boundaries over the same data model. The first keeps independent zero-baseline series; the second requires aligned timestamps and draws additive bands whose final upper edge is the total. Both own axes, native drawing, bounded monotone curves, hover/keyboard inspection, accessibility, a 240-point-per-series cap, two-edge morph animation and synchronous Reduce Motion. Material selects presentation: continuous curves normally, or a segmented spectrum analyzer for Classic Player and imported player skins. Projections may remain explicitly linear and dashed in either material. Both also draw **bars** — grouped or stacked, vertical or as a horizontal ranking — over a categorical axis, on the same geometry: a bar is `baselineY → y` in the space a line already occupies, so composition alone decides grouped versus stacked. A bar stays a bar in every material; the spectrum analyzer renders filled bands, and running a three-category comparison through it answers the question with a column of cells. A bar may also stand on the **time** axis: `ThemedChartSeries.barSpan` states its width as the span of time it measures — the Usage limit chart's one column per day at a five-hour window's highest reading — so the band is that span's share of the resolved range, the column is centred on its bucket, it is never grouped with another series (two span series sharing a bucket would be two claims about one interval), and a bucket that began before the range is clipped to the plot rather than drawn into the value gutter. **A categorical chart's name gutter is part of its hover target**: a category name is a word in a fixed-width slot, so a long one is drawn as an ellipsized stub, and the pointer that goes to read it used to land outside the plot and be answered by nothing. Pointing at the name or at the band it labels states the same lines — the whole name *and* its reading, since a thin bar prints no number of its own — from the one list the accessibility value reads. With nothing to plot the model's `emptyMessage`/`emptyDetail`/`placeholder` reach `ThemedChartPlaceholderView` and the **value axis prints no numbers**: an empty chart labelling its rules 0/0.2/0.5/0.8/1 is the automatic domain describing itself rather than anything anyone measured. See [`usage-dashboard.md`](usage-dashboard.md) and [`mcp-and-display.md`](mcp-and-display.md). |
| `AccountUsageFleetView` / `UsageDashboardView` | Usage opens on Consumption: a large total and top-three tool split beside the shared stacked-band chart, a five-metric strip, virtual breakdown and explicit coverage. A themed two-state control swaps that retained column for Limit history's chooser, reset/projection summary and independent-series chart without rebuilding either tree. Current capacity follows the selected analysis; its virtualized account-card table takes the Settings page's viewport instead of introducing an inner scroller, while the Option-click toolbar popover gives the same table a bounded viewport. Both accept prepared values; account discovery, filesystem scans, CLI exports, provider calls and journal I/O remain outside the design boundary. |
...
| `ThemedScrollView` | An `NSScrollView` that starts transparent — the stock one paints a system surface — and installs themed vertical and horizontal scrollers without enabling either. A period scroller forces legacy-width space because its arrows are permanent furniture. A material may move the vertical scroller to the leading edge; layout mirrors AppKit's reservation. The project tree opts into `.sidebarNavigator`, which resolves an authored fill/bevel and insets the document inside its edge; every other call site stays transparent. A nested viewport states what it does with a vertical-dominant gesture through `verticalScrollHandoff` — `.always` for a code block or a wide table, which have no vertical range of their own, `.atContentEnds` for one that does and hands the rest up — and the gesture is locked to the axis it began on for its whole life, momentum included (`NestedScrollGestureRouter`), because re-deciding from a wobbling tail lets a nested viewport steal the end of a flick aimed past it. A given-away gesture goes to the enclosing scroll view, or up the responder chain when there is none: `NSScrollView` answers a gesture it has no range for by rubber-banding rather than by declining it. `axis = .horizontalOnly` is the stronger statement a strip makes — see `ThemedTabStripView`. |
...
| `ThemedStatusReceiptButton` | A complete check receipt inside a bounded card column. It takes already-localized semantic fragments, wraps only between whole fragments, and grows by full text lines instead of truncating provider outcomes. It owns the ordinary themed-control contract — hover plate, robust press/release, keyboard focus and activation, accessibility, live ink, and optical insets — while the source-control model remains the sole owner of outcome wording and order. |
| `ThemedStatusProgressRing` | A compact semantic ring for bounded successful/non-blocking/active/attention counts. It draws non-blocking results as the neutral track and overlays positive, warning and negative slices; adjacent text must name every non-zero exact outcome so hue is never the only signal. |
...
rows use primary ink. Checks use `ThemedStatusReceiptButton`: every non-zero exact provider outcome
is one indivisible text fragment, line flow occurs only between fragments, and the row grows rather
than clipping at the column cap. Its progress ring summarizes four semantic categories —
successful, non-blocking, active and needs-attention — without collapsing the exact adjacent words.
Coverage fragments also say when a multi-page read is incomplete or capped. The
...
An in-flight structured plan adds one `RunPlanDisclosureView` row immediately after the checkout
row. It never replaces the branch, and an agent with no structured plan adds no empty slot. The
compact row names the current item plus position; its anchored popover owns the exact ordered
checklist and uses reusable table rows, so provider-sized plans do not construct provider-sized
AppKit trees. This surface is deliberately host-only in `HostPopoverCatalog`: extensions may
customize their own status-card rows, while Threading retains plan ingestion, turn-boundary clear,
remote projection, checklist semantics, and disclosure placement.
...
out of the editor and are appended only to the string submitted to the CLI. A movie joins the
same strip where the host opts in (`showsMovieAttachments`, the two chat composers): its tile is
the image tile with the recording's first frame in it and a play mark on an opaque themed plate
— the remove button's plate, for the same reason, since a poster is as likely to be white as
black under it. The frame comes from `MoviePosterFrame`, the extractor the attachments pane's
rows use, off the main actor at `.userInitiated`; a `.mov` the decoder cannot open gives up no
frame and falls back to a path exactly as bytes that are not a picture do. Pressing the tile
opens the inspector with the movie in the player (`.media(format: .video)`, so the rail carries
it and the arrows reach it), and its menu drops the two actions that need a still: there is no
picture to comment on, and the frame worth copying is the one the lightbox stops on. The report
form does not opt in — its backend takes pictures only — so a movie there is refused as before. Raw image data is
...
An embedded table is not automatically edge-to-edge. `soleColumnFillsBoundsExactly` is the
explicit opt-in for a card whose cell owns the complete table width; containment cannot infer it,
because the sidebar outline is embedded too and AppKit's inset band is the space its narrow-width
density deliberately reclaims. The exact policy is applied before AppKit commits a new frame so a
pending tile cannot restore the previous column width.
...
## 2026-08-22 — a row on a plate is not a `List` row, and its swipe is not a `DragGesture`

Archive-by-swipe had been in `SessionDashboard.swift` since `5fa09459`, written as
`.swipeActions(edge: .trailing)`, and had never once worked. `.swipeActions` is a `List`
modifier; the phone's rows sit in a `LazyVStack` on one `ThemedRowGroup` plate, because that is
what draws a grouped table without handing UIKit the row background. Outside a `List` SwiftUI
ignores the modifier — no error, no warning, no runtime complaint. Every assertion anyone would
have written about it would have passed, because the modifier was right there in the source. The
gesture is now ours: `Sources/ThreadingMobile/MobileRowSwipe.swift`.

Three SwiftUI arrangements were built and measured on a phone before the working one, and each
failed differently. They are recorded because each looks correct in a diff.

**A `DragGesture` inside a `ScrollView` takes the scroll, whatever you attach it with.** Not just
`highPriorityGesture` — `gesture` and `simultaneousGesture` too. A `DragGesture` recognises in
every direction, and the scroll view is told to wait for it; a drag begun on a row therefore
scrolled the dashboard nowhere, while the same drag begun on the banner above the rows scrolled
normally. That asymmetry is how it was caught, and a pixel diff of a left-hand strip is what
proved it, because the screenshots differ by a clock digit and an animating orb whether the list
moved or not. Ignoring a vertical drag in the handler does not help: recognising is what costs the
scroll, not acting. A component that quietly eats the list's scroll is a worse bug than the one
...
before claiming it. `gestureRecognizerShouldBegin` answers on velocity alone, because at that
moment the finger has barely moved: sideways is the row's, anything else is the list's, and a
declined recogniser is one `panGestureRecognizer.require(toFail:)` no longer waits for.

**A SwiftUI `Button` fires at the end of a swipe.** A button's press survives any drag that stays
inside its own bounds, and a full-width row's bounds contain the whole gesture — so the row
archived *and* opened the chat. Nothing available to a gesture retracts a press already in
flight: not `highPriorityGesture`, not withdrawing `allowsHitTesting`, not swapping the
`GestureMask` mid-drag. All three were tried against this row. The row is no longer a button; its
tap is a `TapGesture` the seam owns, which fails the moment the finger travels, and the traits
the button used to publish (`.isLink`, one combined element, a default activation action) are
stated on the row instead.

**The strip is clipped to what the row uncovers**, rather than laid under it. A row on this plate
paints no background of its own — the group paints `panel` exactly once, which is what keeps a
translucent panel from doubling — so a strip lying under the row would show straight through the
words. And it is drawn the way this app draws destruction everywhere else: the theme's `negative`
as *ink* on a `controlResting` plate, not a saturated red slab. A drop affordance already shipped
here once as a loud plate, louder than the window's own selection; a swipe is not the place to
re-learn that. The plate changes to `selection` only once letting go would archive rather than
open, which with a haptic is the one warning a quiet strip gives.

What is testable is arithmetic, and `MobileRowSwipe` is where it lives with no view attached:
where the row sits for a finger this far along, how far a throw is read ahead, and whether letting
...
rectangles separately, and the SwiftUI fallback uses the same fixed slot. The slot is stable, not
inert: crossing the full-swipe threshold changes the ground to `selection`, fires the haptic and
gives the action symbol one non-repeating bounce. Crossing back restores the quiet plate without
replaying the symbol. The reusable UIKit cell and SwiftUI fallback both keep that edge, while
Reduce Motion applies the colour endpoint immediately and omits the bounce.
...
The ground means the face immediately behind the ink, not merely the window backdrop. A component
that paints translucent `surface` and `surfaceHover` roles between its content and that backdrop
must composite both faces first and hold one foreground ladder over both. `Design.Ink.legible(over:)`
is that operation. The account-usage pill is the concrete case: Pure over a black terminal gives
the hovered pill a `#3D3D3D` face, on which text and a progress track calibrated only for black fall
back below their floors exactly while the usage popover is open.
...
could meet its border exactly at the horizontal midpoint. In System dark the row and border were
nearly the same grey, so the row read as a rectangular background protruding through both sides of
the circle. This was not a one-pixel radius leak: content outside a correctly clipped control can
still become part of its perceived silhouette when there is no separating ink.
