...
| `WindowChromeButton` | A takeover window's Window menu/close/minimize/zoom, one component for all four roles and every glyph family (`squares`, `platinum`, `beos`, `openstep`, `irix`, `plain`) — the tab strip's "every" lesson applied to period chrome. Calls the *semantic* window operations, because the `perform*` forms animate a standard button a frameless window does not have and refuse outright; zoom follows the window and becomes Restore while maximized, while Window menu opens app-owned `ThemedMenuPresenter` rows. Full `ThemedControl` contract: keyboard, focus ring, AX press. |
