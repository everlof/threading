...
| Resolved | Live run plans in terminal, Chat and remote clients | Terminal hooks and transcript catch-up share one serial off-main reducer and call-id dedupe set. JSONL scanning resumes from a persistent byte cursor, reads bounded passes, and resets only when the file changes identity or truncates; ANSI and emulator rows are never scanned. The compact Mac surfaces retain one summary. Full Mac checklists use reusable table rows, while remote clients receive one summary plus revision-bound 64-row pages and iOS materializes them through `LazyVStack`. Turn end sends an authoritative clear rather than retaining a historical plan. |
