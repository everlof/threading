...
Tool results are plain text except for `browser_screenshot`. Display tools have already drawn
their image, so returning it to the model would spend image tokens for no benefit.
`browser_screenshot` is different: visual inspection is the tool's purpose, so it returns a
standard MCP image block as well as caching and optionally displaying the PNG.
