...
/// One MCP invocation forwarded by Threading to the extension that registered the tool.
///
/// `sessionID` is an opaque routing identifier. It lets an extension correlate cache entries or
/// future host-data requests with the conversation that called it without exposing Threading's
...
/// Threading's built-in tools and keeps results cheap for the calling conversation.
