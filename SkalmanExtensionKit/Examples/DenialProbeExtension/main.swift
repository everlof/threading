...
/// allow. That matters: `--skalman-register` and `--skalman-serve` are the only entry modes the
...
enum DenialProbe {
    static let secretService =
        "se.mjukis.Skalman.extension-secrets.v1.se.mjukis.skalmantest.probe"
    static let secretAccount = "containment-probe"
    static let adversarialKeychainEnvironment =
        "SKALMAN_EXTENSION_ADVERSARIAL_KEYCHAIN_PROBE"
    ...
    static func run() -> [Finding] {
        ...
        return [
            ...
            Finding(
                ...
                allowed: canRead(
                    home + "/Library/Application Support/Skalman/skalman.db"
                )
            ),
            ...
            Finding(
                ...
                allowed: canReadDirectory(
                    home + "/Library/Application Support/Skalman/Extensions"
                )
            ),
            Finding(name: "writesOwnPackage", allowed: canWrite(packagePath + ".written")),
            Finding(name: "writesTemporaryDirectory", allowed: canWrite("/tmp/skalman-probe")),
...
case "--skalman-register", "--skalman-serve", nil:
