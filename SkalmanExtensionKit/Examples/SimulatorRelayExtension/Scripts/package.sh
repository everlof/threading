...
if [ -n "${SKALMAN_SWIFT_EXEC:-}" ]; then
  SWIFT_COMMAND="$SKALMAN_SWIFT_EXEC"
...
OUTPUT="$OUTPUT_ROOT/se.mjukis.simulator-relay.skalmanextension"
...
SKALMAN_EXTENSION_SDK_PATH="$SDK_ROOT" "$SWIFT_COMMAND" build \
  --disable-sandbox \
...
CORE_BIN_DIR="$(SKALMAN_EXTENSION_SDK_PATH="$SDK_ROOT" "$SWIFT_COMMAND" build \
  --package-path "$PROJECT_DIR" \
  --swift-sdk "$SDK_ID" \
  --show-bin-path)"
COMPANION_BIN_DIR="$(SKALMAN_EXTENSION_SDK_PATH="$SDK_ROOT" "$SWIFT_COMMAND" build \
  --package-path "$SDK_ROOT" \
  --show-bin-path)"
...
rsync -a \
  ...
  --exclude .swiftpm \
  --exclude .DS_Store \
  "$SDK_ROOT/" "$STAGING/Source/Vendor/SkalmanExtensionKit/"
rsync -a \
  --exclude .DS_Store \
