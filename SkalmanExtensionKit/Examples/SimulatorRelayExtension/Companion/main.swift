...
private final class SimulatorRelay: @unchecked Sendable {
    ...
    private let captureQueue = DispatchQueue(
        label: "se.mjukis.simulator-relay.capture",
        qos: .userInteractive
    )
            ...
            if consecutiveCaptureFailures == 80 {
                fail(
                    "Simulator Relay could not capture the Simulator window. "
                        + "Allow Screen Recording for Skalman and reload "
                        + "the extension."
                )
            }
            return
        ...
        guard CGPreflightScreenCaptureAccess() else {
            fail(
                "Simulator Relay needs Screen Recording. macOS attributes this direct child "
                    + "process to Skalman, so allow Skalman in System Settings → Privacy & "
                    + "Security → Screen & System Audio Recording, then relaunch Skalman."
            )
        }
    }
        ...
        guard AXIsProcessTrustedWithOptions(checkOnly) else {
            fail(
                "Simulator Relay needs Accessibility for input relay. macOS attributes this "
                    + "direct child process to Skalman, so allow Skalman in System Settings → "
                    + "Privacy & Security → Accessibility, then relaunch Skalman."
            )
        }
    }
}
...
guard CommandLine.arguments.dropFirst().first == "--skalman-companion-serve",
    ...
    fail("Simulator Relay was not launched as a Skalman companion.", status: 64)
}
