...
@main
struct SkalmanComponentCatalogGenerator {
    static func main() throws {
        ...
        guard positional.count == 1 else {
            FileHandle.standardError.write(
                Data("usage: SkalmanComponentCatalogGenerator [--check] OUTPUT_DIRECTORY\n".utf8)
            )
            Darwin.exit(64)
        }
        ...
        var outputs: [URL: Data] = [
            outputDirectory.appendingPathComponent("component-catalog.json"):
                try encoder.encode(SkalmanComponentCatalog.document),
            outputDirectory.appendingPathComponent("component-catalog.md"):
                Data(SkalmanComponentCatalog.documentationMarkdown().utf8)
        ]

        for description in SkalmanComponentCatalog.document.components {
