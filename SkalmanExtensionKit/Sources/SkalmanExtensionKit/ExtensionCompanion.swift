...
/// companion may produce a remote surface or perform an OS operation only after Skalman grants
...
///
/// The schemas make the crossing inspectable without exposing an arbitrary pipe. Skalman still
...
/// When Skalman may run a companion.
///
...
/// window does not thereby gain project, session, account, storage, or other Skalman host data.
...
public enum SkalmanCompanionAPI {
...
///
/// Skalman supplies the expected ID and generation in the launch environment and verifies both
...
public struct ExtensionCompanionHello: Codable, Equatable, Sendable {
    ...
    public init(
        protocolVersion: Int = SkalmanCompanionAPI.protocolVersion,
        companionID: String,
        generation: String
    ) {
...
/// A bounded control message written by Skalman to a companion's stdin.
///
...
public enum ExtensionCompanionEnvironment {
    public static let extensionIdentifier = "SKALMAN_EXTENSION_ID"
    public static let companionIdentifier = "SKALMAN_COMPANION_ID"
    public static let generation = "SKALMAN_COMPANION_GENERATION"
    public static let capabilitiesJSON = "SKALMAN_COMPANION_CAPABILITIES_JSON"
    /// Dedicated full-duplex binary socket for declared remote surfaces. Absent when the
    /// companion declares no surfaces.
    public static let remoteSurfaceDescriptor = "SKALMAN_COMPANION_SURFACE_FD"
}
