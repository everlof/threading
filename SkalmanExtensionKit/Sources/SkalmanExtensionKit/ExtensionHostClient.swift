...
/// One complete, atomic component-state publication to Skalman's extension host.
///
...
public struct ExtensionHostConnection: Equatable, Sendable {
    public static let urlEnvironmentKey = "SKALMAN_EXTENSION_HOST_URL"
    public static let tokenEnvironmentKey = "SKALMAN_EXTENSION_HOST_TOKEN"
    ...
    public static let descriptorEnvironmentKey = "SKALMAN_EXTENSION_HOST_FD"
    ...
    public static let descriptorBaseURL = URL(string: "http://skalman-extension-host/v1")!
...
public enum ExtensionHostClientError: Error, Equatable, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .configurationUnavailable:
            return "Skalman did not grant this process an extension-host connection."
        case .invalidResponse:
            return "Skalman returned an invalid extension-host response."
        case .rejected(let status, let message):
            return "Skalman rejected the extension-host request (\(status)): \(message)"
        case .hostClosed:
            return "Skalman closed this extension's host connection."
        }
    }
}
...
public struct ExtensionHostClient: Sendable {
    ...
    /// Reads only the process and listening-port rows Skalman attributes to this session.
    ///
    ...
    ///
    /// Skalman's bearer token supplies caller identity. The provider receives that verified
    ...
    ///
    /// The core sends its normal host authority to Skalman, never to the companion. Skalman
    ...
    private func request(
        ...
        // one `skalman.host_exchange` import.
