...
/// Environment variables Skalman supplies only for declared storage capabilities.
///
...
public enum ExtensionStorageEnvironment {
    public static let keyValueDirectory = "SKALMAN_EXTENSION_KEY_VALUE_DIRECTORY"
    public static let cacheDirectory = "SKALMAN_EXTENSION_CACHE_DIRECTORY"
}
...
/// experimental `sandbox-exec` launcher Skalman grants one directory and the extension writes a
...
/// durable large-file surface, which this deliberately is not: Skalman may delete any entry at
/// any moment.
