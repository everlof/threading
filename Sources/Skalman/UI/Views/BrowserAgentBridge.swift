...
enum BrowserAgentScripts {
    ...
    static let navigationReadiness = #"""
        (() => {
          ...
          globalThis.__skalmanNavigationReadinessToken = token;
          const report = () => {
            try {
              webkit.messageHandlers.skalmanNavigationReadiness.postMessage({
    ...
    static let snapshot = #"""
        ...
        let state = globalThis.__skalmanAgentState;
        if (!state || state.document !== document) {
          ...
          globalThis.__skalmanAgentState = state;
        }
    ...
    private static let targetPrelude = #"""
        ...
        function deepQuerySelector(value) {
          ...
          if (matchCount > 1) {
            ...
            error.name = 'SkalmanAmbiguousSelectorError';
            throw error;
          }
          return match;
        }
        ...
        function resolveTargetValues(targetRef, targetSelector, targetLocator = null) {
          ...
          if (targetSelector) {
            try {
              ...
              targetResolutionError = error?.name === 'SkalmanAmbiguousSelectorError'
                ? error.message
    ...
    static let hover = targetPrelude + #"""
        ...
        const hoverAttribute = 'data-skalman-agent-hover';
        ...
        for (const targetDocument of documents) {
          ...
          if (!hoverStyle?.isConnected) {
            ...
            hoverStyle.setAttribute('data-skalman-agent-hover-styles', '');
    ...
    static let consoleCapture = #"""
        (() => {
          if (globalThis.__skalmanConsoleInstalled) return;
          globalThis.__skalmanConsoleInstalled = true;
          const post = payload => {
            try { globalThis.webkit?.messageHandlers?.skalmanConsole?.postMessage(payload); }
            catch (_) {}
          };
          const render = value => {
    ...
    static let networkCapture = #"""
        (() => {
          if (globalThis.__skalmanNetworkInstalled) return;
          globalThis.__skalmanNetworkInstalled = true;

          const post = payload => {
            try { globalThis.webkit?.messageHandlers?.skalmanNetwork?.postMessage(payload); }
            catch (_) {}
          };
