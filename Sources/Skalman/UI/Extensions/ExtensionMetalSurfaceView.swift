...
///
/// The extension supplies one pure fragment function. Skalman supplies the vertex stage,
...
final class ExtensionMetalSurfaceView: MTKView, MTKViewDelegate {
    ...
    private static let hostVertexFunction = "skalmanHostSurfaceVertex"
    private static let hostFragmentFunction = "skalmanHostSurfaceFragment"
    ...
    private static func completeSource(
        ...
        struct SkalmanSurfaceVertexOut {
            float4 position [[position]];
            float2 uv;
        };

        vertex SkalmanSurfaceVertexOut \(hostVertexFunction)(uint vertexID [[vertex_id]]) {
            const float2 positions[3] = {
                float2(-1.0, -1.0),
                float2( 3.0, -1.0),
                float2(-1.0,  3.0)
            };
            SkalmanSurfaceVertexOut out;
        ...
        fragment float4 \(hostFragmentFunction)(
            SkalmanSurfaceVertexOut in [[stage_in]],
            constant SkalmanSurfaceUniforms &uniforms [[buffer(0)]]
        ) {
