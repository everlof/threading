...
/// Launches a Swift-compiled WebAssembly extension in Skalman's signed interpreter process.
///
...
/// filesystem preopens and no network API; its only Skalman import forwards one authenticated
/// request over descriptor 3.
struct WasmLaunchPolicy: ExtensionLaunchPolicy {
    static let runnerName = "skalman-wasm-extension-runner"
    ...
    func spawn(_ request: ExtensionLaunchRequest) throws -> ExtensionChildProcess {
        ...
        guard request.arguments == ["--skalman-register"]
                || request.arguments == ["--skalman-serve"] else {
...
struct RuntimeSelectingLaunchPolicy: ExtensionLaunchPolicy {
    ...
    private func policy(for bundle: SkalmanExtensionBundle) -> ExtensionLaunchPolicy {
