...
final class ExtensionHostService {
    ...
    private let queue = DispatchQueue(
        label: "se.mjukis.skalman.extension-host",
        qos: .userInitiated
    )
    ...
    init(
        ...
        identityRegistry: ExtensionIdentityResolverRegistry = .shared,
        serviceRouter: ExtensionServiceRouting? = nil,
        companionRouter: ExtensionCompanionRouting? = nil,
        secretStore: ExtensionSecretStoring = KeychainExtensionSecretStore.shared,
    ...
    /// ordinary answer here — Skalman may reclaim any entry at any moment, and an extension
        ...
        do {
            ...
            if publication.patches.contains(where: Self.patchUsesUnsupportedSignal) {
                respond(jsonFailure(
                    status: 422,
                    reason: "Unprocessable Content",
                    "The component hook uses a host signal this version of Skalman does not provide."
                ))
                return
            }
