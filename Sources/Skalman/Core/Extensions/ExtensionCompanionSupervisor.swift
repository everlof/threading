...
struct ExtensionCompanionLaunchRequest {
    let companion: SkalmanExtensionCompanionBundle
...
protocol ExtensionCompanionLaunchPolicy {
...
final class ExtensionCompanionSupervisor: @unchecked Sendable {
    ...
    typealias Validator = (
        SkalmanExtensionCompanionBundle,
        SkalmanExtensionBundle
    ...
    private let readQueue = DispatchQueue(
        label: "se.mjukis.skalman.extension-companion.stdout",
        qos: .userInitiated
    )
    private let writeQueue = DispatchQueue(
        label: "se.mjukis.skalman.extension-companion.stdin",
        qos: .userInitiated
    )
    ...
    private init(
        ...
        // writing an operation or graceful shutdown to its closed stdin terminates Skalman with
        ...
        do {
            child = try policy.spawn(ExtensionCompanionLaunchRequest(
                ...
                arguments: ["--skalman-companion-serve"],
    ...
    private func handleProtocolLine(_ bytes: Data.SubSequence) {
        ...
        do {
            ...
            guard hello.protocolVersion == SkalmanCompanionAPI.protocolVersion else {
