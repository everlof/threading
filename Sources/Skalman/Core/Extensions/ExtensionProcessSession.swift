...
enum ExtensionProcessError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .writeFailed(let message):
            return "Skalman could not send an action to the extension: \(message)"
        }
    }
}
...
final class ExtensionProcessSession: @unchecked Sendable {
    ...
    private let bundle: SkalmanExtensionBundle
    ...
    private let readQueue = DispatchQueue(
        label: "se.mjukis.skalman.extension.stdout",
        qos: .userInitiated
    )
    private let writeQueue = DispatchQueue(
        label: "se.mjukis.skalman.extension.stdin",
        qos: .userInitiated
    )
