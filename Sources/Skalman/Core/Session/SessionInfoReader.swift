...
///
/// A session can run processes from two places at once: the agent Skalman launched, and the
...
final class SessionInfoReader {
    ...
    private let queue = DispatchQueue(label: "com.skalman.session-info", qos: .userInitiated)
