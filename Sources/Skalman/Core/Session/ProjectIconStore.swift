...
enum ProjectIconStore {
    ...
    static func store(imageData: Data, for projectID: ProjectID) -> String? {
        ...
        do {
            ...
            SkalmanLogger.agent.error("Failed to store project icon: \(error.localizedDescription, privacy: .public)")
            return nil
        }
    ...
    /// ImageIO caps at `storedPixelSize` without upscaling anything smaller. Internal so
    ...
    static func normalizedPNGData(from data: Data) -> Data? {
        ...
        let options: [CFString: Any] = [
            ...
            kCGImageSourceThumbnailMaxPixelSize: ProjectIconDefaults.storedPixelSize
        ]
