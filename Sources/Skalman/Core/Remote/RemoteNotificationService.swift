...
/// development and self-hosted builds point Skalman at an Apple `.p8` key through environment
...
actor RemoteAPNSPushSender {
    ...
    nonisolated static func fromEnvironment(
        ...
        guard let keyID = environment["SKALMAN_APNS_KEY_ID"]?.nilIfEmpty,
              let teamID = environment["SKALMAN_APNS_TEAM_ID"]?.nilIfEmpty,
              let path = environment["SKALMAN_APNS_PRIVATE_KEY_PATH"]?.nilIfEmpty,
        ...
        return RemoteAPNSPushSender(configuration: Configuration(
            keyID: keyID,
            teamID: teamID,
            topic: environment["SKALMAN_APNS_TOPIC"]?.nilIfEmpty
                ?? "se.mjukis.Skalman.mobile",
