...
/// a new release of its CLI a data-corruption risk. Instead Skalman copies the source transcript
...
enum ConversationContinuation {
    ...
    static func openingPrompt(for session: AgentSession) -> String? {
        ...
        return """
            This is a Skalman cross-provider continuation bootstrap, not a new user request. \
            Before answering, call the Skalman MCP tool `conversation_history` with no cursor, \
