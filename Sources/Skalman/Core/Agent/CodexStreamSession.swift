...
/// process subscribed to the thread graph, which gives Skalman both a reusable conversation
...
final class CodexStreamSession: ConversationStreamSession, SubagentReportingConversation {
    ...
    private func sendPendingTurnIfReady() {
        ...
        guard sendRequest(
            ...
            finishTurnWithTransportError("Skalman could not send the Codex turn.")
            return
        }
        pendingPrompt = nil
    }
    ...
    private func handleServerRequest(
        ...
        default:
            sendResponse(
                id: id,
                error: ["code": -32601, "message": "Skalman does not handle \(method)."]
            )
            return
        }
