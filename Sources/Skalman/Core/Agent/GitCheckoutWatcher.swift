...
/// duplicate callback back at the cost of a registry with lifetimes to get wrong.
final class GitCheckoutWatcher {
    ...
    private let queue = DispatchQueue(label: "com.skalman.git-watch", qos: .utility)
    ...
    private func isRelevant(_ path: String, flags: FSEventStreamEventFlags) -> Bool {
        ...
        return GitWatchFilter.isRelevant(path, gitDirectories: gitDirectories)
    }
