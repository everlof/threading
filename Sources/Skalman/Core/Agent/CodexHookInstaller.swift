...
/// Installs Skalman's lifecycle hooks into a Codex account's `hooks.json`.
///
...
enum CodexHookInstaller {
    ...
    /// Ensures the account's `hooks.json` carries Skalman's current entries.
    ///
    ...
    static func install(inCodexHome codexHome: String) -> Bool {
        ...
        guard !isEquivalent(merged, existing) else {
            SkalmanLogger.agent.debug("Codex hooks already current in \(codexHome, privacy: .public)")
            return false
        }
        ...
        do {
            ...
            SkalmanLogger.agent.info("Installed Codex hooks in \(codexHome, privacy: .public)")
            ...
            SkalmanLogger.agent.error("Failed to install Codex hooks: \(error.localizedDescription)")
    ...
    /// Removes Skalman's entries from an account's `hooks.json`, leaving every other tool's.
    @discardableResult
    ...
    /// The command the `PreToolUse` hook runs, which asks Skalman whether a tool may proceed.
    ///
    ...
    /// every session under the account, but brokering suits only the sessions Skalman renders
    ...
    /// Builds the merged document: every foreign entry kept, every Skalman entry rewritten.
    ...
    private static func foreignEntries(in value: Any?) -> [Any] {
        ...
        return entries.filter { !isSkalmanEntry($0) }
    }

    private static func isSkalmanEntry(_ entry: Any) -> Bool {
    ...
    private enum Key {
        ...
        static let payloadVariable = "skalman_payload"
    }
}
