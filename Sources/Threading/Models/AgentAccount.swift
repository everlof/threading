...
struct AgentAccount: Equatable, Identifiable {
    ...
    /// Whether the account is offered for new work.
    ///
    /// A switched-off login is still discovered, still named, and still resolves for the
    /// sessions already on it — a conversation is stored per account, so hiding the account
    /// would orphan its transcripts. What it no longer does is appear anywhere an account is
    /// *chosen*.
    let isEnabled: Bool
