...
///
/// Native plugins cannot build against Threading's private design-system classes. They receive
/// the public `PluginTheme` value instead and own the AppKit or SwiftUI tree they return. SwiftUI
/// expands even a plain `TextField` and `ScrollView` into private AppKit controls, so auditing that
/// foreign subtree as app-authored UI would reject every useful SwiftUI plugin after it loaded.
///
