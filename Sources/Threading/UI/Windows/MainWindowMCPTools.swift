...
final class AgentToolCoordinator: AgentCommandHandling {
    ...
    func handle(_ call: MCPToolCall, for sessionID: SessionID, completion: @escaping @MainActor @Sendable (MCPToolResult) -> Void) {
        let traceStartedAt = Date()
        let shouldTrace = call.builtInTool?.family == .browser
