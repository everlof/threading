...
enum ExtensionHelperError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .helperUnavailable(let name):
            return L10n.format(
                "Threading’s contained extension launcher (%@) is missing from the application bundle.",
                name
            )
        }
    }
...
/// was asked to run and `execve`s the extension **in place**. The pid Threading spawned is
...
struct HelperLaunchPolicy: ExtensionLaunchPolicy {
    ...
    static let helperName = "threading-extension-helper"
    static let networkHelperName = "threading-extension-helper-network"
