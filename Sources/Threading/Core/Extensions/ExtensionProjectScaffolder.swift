...
enum ExtensionProjectScaffolderError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .sdkSnapshotMissing(let path):
            return L10n.format(
                "Threading’s embedded extension SDK is missing or incomplete at %@.",
                path
            )
        case .sdkDocumentationMissing(let path):
            return L10n.format(
                "Threading’s embedded extension authoring documentation is missing or incomplete at %@.",
                path
            )
...
/// `Vendor/`, so the result has no path dependency on Threading's checkout and no network
...
enum ExtensionProjectScaffolder {
    static func scaffold(
        ...
        let staging = destination.deletingLastPathComponent().appendingPathComponent(
            ".threading-scaffold-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    private static func readme(
        ...
        Safe Threading WebAssembly extension scaffold using vendored SDK \(sdkVersion).
        ...
        extension APIs from Threading's application internals.
        ...
        4. Build and assemble an immutable source-bundled package with:
           ...
           Set `THREADING_SWIFT_EXEC=/absolute/path/to/swift` when the selected toolchain is not
           first on `PATH`. The result is `Build/\(identifier).threadingextension`.

        5. Ask Threading's `extension_propose_install` MCP tool to review that package. Threading
    ...
    private static func packagingScript(identifier: String) -> String {
        ...
        SWIFT_COMMAND="${THREADING_SWIFT_EXEC:-swift}"
        OUTPUT_ROOT="${2:-"$PROJECT_DIR/Build"}"
        PACKAGE_NAME="\(identifier).threadingextension"
        ...
        mkdir -p "$OUTPUT_ROOT"
        STAGING="$(mktemp -d "$OUTPUT_ROOT/.threading-package.XXXXXX")"
