...
/// One project folder whose agent sessions run on a Linux host.
///
/// A developer setting for now (`AppSettings.developerRemoteExecutionHosts`): the persisted
/// `ExecutionHost` on `Project` that slice 3 of `remote-execution-hosts.md` describes replaces it
/// once host profiles exist. A project is bound to one host; its sessions do not choose.
struct RemoteExecutionHostAssignment: Equatable, Codable, Sendable {
    /// The project's folder on this Mac, which is how a project is found.
    let projectFolder: String
    let destination: RemoteHostDestination
    /// The checkout on the host that sessions run in.
    ...
    init(projectFolder: String, destination: RemoteHostDestination, remoteDirectory: String) {
    ...
    init(from decoder: Decoder) throws {
        ...
        projectFolder = try container.decode(String.self, forKey: .projectFolder)
        destination = RemoteHostDestination(
            alias: try container.decode(String.self, forKey: .destination),
            configFile: try container.decodeIfPresent(String.self, forKey: .sshConfigFile)
        )
        remoteDirectory = try container.decode(String.self, forKey: .remoteDirectory)
    }

    func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(projectFolder, forKey: .projectFolder)
        try container.encode(destination.alias, forKey: .destination)
        try container.encodeIfPresent(destination.configFile, forKey: .sshConfigFile)
        try container.encode(remoteDirectory, forKey: .remoteDirectory)
    }

    var isValid: Bool {
        destination.isValid
            && remoteDirectory.hasPrefix("/")
            && !remoteDirectory.contains("\0")
            && !projectFolder.isEmpty
    }

    /// Reads the setting's JSON array. Invalid entries are dropped and counted, never guessed at:
    /// a malformed destination must not become a launch somewhere else.
    static func decodeList(_ json: String) -> (assignments: [RemoteExecutionHostAssignment], rejected: Int) {
        guard !json.isEmpty,
              let decoded = try? JSONDecoder().decode([RemoteExecutionHostAssignment].self, from: Data(json.utf8)) else {
            return ([], json.isEmpty ? 0 : 1)
        }
        let valid = decoded.filter(\.isValid)
        return (valid, decoded.count - valid.count)
    }

    /// The assignment for a project folder, matched exactly after standardizing both paths.
    static func assignment(
        forProjectFolder folder: String,
        in assignments: [RemoteExecutionHostAssignment]
    ) -> RemoteExecutionHostAssignment? {
        let wanted = (folder as NSString).standardizingPath
        return assignments.first { ($0.projectFolder as NSString).standardizingPath == wanted }
    }
}
