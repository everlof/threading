...
public enum ThreadingWasmRuntimeError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .invalidHostCall:
            return "The WebAssembly extension made an invalid Threading host call."
...
/// Runs one WASI command module with no filesystem preopens and one optional Threading host call.
///
...
/// `threading.host_exchange`. Registration deliberately passes no broker descriptor, making the
/// import fail closed if extension code attempts a host call before it is authorised.
public enum ThreadingWasmRuntime {
    public static let hostModule = "threading"
    ...
    /// entitlement: Threading opens the validated module and installs that one descriptor.
    @discardableResult
    public static func run(
        moduleBytes: Data,
        moduleName: String,
        ...
        store.resourceLimiter = ThreadingResourceLimiter()
        ...
        imports.define(
            ...
            Function(
                ...
                guard values.count == 4,
                    ...
                    throw ThreadingWasmRuntimeError.invalidHostCall
                }
...
private struct ThreadingResourceLimiter: ResourceLimiter {
    func limitMemoryGrowth(to desired: Int) throws -> Bool {
        desired <= ThreadingWasmRuntime.maximumGuestMemoryBytes
    }

    func limitTableGrowth(to desired: Int) throws -> Bool {
        desired <= ThreadingWasmRuntime.maximumTableElements
    }
}
...
final class HostDescriptorExchange {
    ...
    func exchange(_ request: Data) throws -> Data {
        ...
        while true {
            ...
            guard responseBuffer.count <= ThreadingWasmRuntime.maximumResponseBytes else {
                throw ThreadingWasmRuntimeError.invalidHostCall
            }
    ...
    private func completeResponseLength(in data: Data) throws -> Int? {
        ...
        guard !overflow, total <= ThreadingWasmRuntime.maximumResponseBytes else {
            throw ThreadingWasmRuntimeError.invalidHostCall
        }
